...
void hal_rcc_reset(void)
{
    ...
    RCC->CR |= 0x00000001u;         /**< Set HSION bit first -> keep cpu running */
    RCC->CR &= 0xeaf6ffffu;         /**< Reset HSEON, CSSON, PLLON, PLLI2S, */
    ...
    RCC->PLLSAICFGR = 0x24003000u;  /**< only STM32F42xx/43xx) */
    ...
    RCC->APB1RSTR = 0u;
    RCC->APB2RSTR = 0u;
    RCC->AHB1ENR = 0x00100000u;
    RCC->AHB2ENR = 0u;
    RCC->AHB3ENR = 0u;
    RCC->APB1ENR = 0u;
    RCC->APB2ENR = 0u;
    RCC->AHB1LPENR = 0x7e6791ffu;
    RCC->AHB2LPENR = 0x000000f1u;
    RCC->AHB3LPENR = 0x00000001u;
    RCC->APB1LPENR = 0x36fec9ffu;
    RCC->APB2LPENR = 0x00075f33u;
...
void hal_rcc_set_peripheral(hal_peripheral_t peripheral, hal_bool_t status)
{
    ...
    assert(peripheral >= 0 && peripheral <= PER_TIM5);
    switch (peripheral) {
        /* AHB1 */
        case PER_GPIOA:
            bit_pos = 0u;
...
void hal_rcc_setup_pll(hal_rcc_osc_t pll, hal_rcc_pll_init_t init)
{
    ...
    /* Configure pll */
    assert( pll == HAL_RCC_OSC_HSI || pll == HAL_RCC_OSC_HSE || 
            pll == HAL_RCC_OSC_PLL || pll == HAL_RCC_OSC_PLLI2S || 
            pll == HAL_RCC_OSC_PLLSAI);
    switch (pll) {
        case HAL_RCC_OSC_PLL:
            RCC->PLLCFGR &= ~0x0f037fc0u;
            RCC->PLLCFGR |= ((uint32_t) init.n_factor << 6u);
            RCC->PLLCFGR |= ((((uint32_t) init.p_divider - 1u) >> 1u) << 16u);
            RCC->PLLCFGR |= ((uint32_t) init.q_divider << 24u);
            break;
        
        case HAL_RCC_OSC_PLLI2S:
            RCC->PLLI2SCFGR &= ~0x7f007fc0u;
            RCC->PLLI2SCFGR |= ((uint32_t) init.n_factor << 6u);
            RCC->PLLI2SCFGR |= ((uint32_t) init.q_divider << 24u);
            RCC->PLLI2SCFGR |= ((uint32_t) init.r_divider << 28u);
            break;
        
/*        case HAL_RCC_OSC_PLLSAI:
            RCC->PLLSAICFGR &= ~0x7f007fc0u;
            RCC->PLLSAICFGR |= ((uint32_t) init.n_factor << 6u);
            RCC->PLLSAICFGR |= ((uint32_t) init.q_divider << 24u);
            RCC->PLLSAICFGR |= ((uint32_t) init.r_divider << 28u);
            break;
*/        
        case HAL_RCC_OSC_PLLSAI:
...
void hal_rcc_setup_clock(hal_rcc_clk_init_t init)
{
    ...
    RCC->CFGR &= ~0x0000fcf0u;
    RCC->CFGR |= ((uint32_t) init.hpre << 4u);
    RCC->CFGR |= ((uint32_t) init.ppre1 << 10u);
    RCC->CFGR |= ((uint32_t) init.ppre2 << 13u);
    
    /* Select system clock source */
    RCC->CFGR &= ~0x00000003u;

    assert( init.osc == HAL_RCC_OSC_HSI || init.osc == HAL_RCC_OSC_HSE || 
            init.osc == HAL_RCC_OSC_PLL || init.osc == HAL_RCC_OSC_PLLI2S || 
            init.osc == HAL_RCC_OSC_PLLSAI);
    switch (init.osc) {
        case HAL_RCC_OSC_HSI:
            reg = 0u;
            break;
    ...
    while ((RCC->CFGR & 0x0000000cu) != (reg << 2u));
#endif
}
