...
#define CHECK_ARRAY_OUT_OF_BOUNDS_COMPARE        0x04u
                         ...
                         hal_timer_base_init_t init)
{
    ...
    /* process mode */
    reg = ((uint32_t) init.mode << 4u) |
          ((uint32_t) init.run_mode << 3u);
    timer->CR1 &= ~(0x78u);
    timer->CR1 |= reg;
    ...
    /* process master mode */
    timer->CR2 &= ~(0x7u << 4u);
    timer->CR2 |= ((uint32_t) init.master_mode << 4u);
}
...
{    
    ...
    if (init.source == HAL_TIMER_CLKSRC_ETRF) {
        timer->CR1 |= ((uint32_t) init.filter_div << 8u);
        timer->SMCR |= ((uint32_t) init.filter.prescaler << 12u);
        timer->SMCR |= ((uint32_t) init.filter.mode << 8u);
        timer->SMCR |= ((uint32_t) init.filter.polarity << 15u);
    }
    
    /* Trigger source */
    timer->SMCR |= ((uint32_t) init.trg_source << 4u);
                          ...
                          hal_timer_input_init_t init)
{
    ...
    /* process input */
    reg = ((uint32_t) init.input << 0u) |              /**< CCMR: CCyS */
          ((uint32_t) init.filter.prescaler << 2u) |   /**< CCMR: ICyPSC */
          ((uint32_t) init.filter.mode << 4u);         /**< CCMR: ICyF */
    // prescaler
                           ...
                           hal_timer_output_init_t init)
{
    ...
    reg = ((uint32_t) init.mode << 4u);                 /**< CCMR: OCyM */
    set_channel_ccreg(timer, channel, reg);
...
{
    uint8_t bit_pos = 0u;
    uint8_t ch_pos = (channel & 0xfu) * 4u;
    ...
    if (status == ENABLE) {
        timer->CCER |= ((0x1u << bit_pos) << ch_pos);
    } else {
        timer->CCER &= ~((0x1u << bit_pos) << ch_pos);
    }        
}
...
static irq_param_t get_irq_param(reg_tim_t *timer, hal_timer_irq_t irq)
{
    ...
    if (timer == TIM1) {
        ...
        assert(irq == HAL_TIMER_IRQ_UE || irq == HAL_TIMER_IRQ_CC1 || 
                irq == HAL_TIMER_IRQ_CC2 || irq == HAL_TIMER_IRQ_CC3 || 
                irq == HAL_TIMER_IRQ_CC4 || irq == HAL_TIMER_IRQ_TRIGGER);
        switch(irq) {
            case HAL_TIMER_IRQ_UE:
                params.nvic_irq_mask = (0x1u << 25u);
                break;
            case HAL_TIMER_IRQ_CC1:
            case HAL_TIMER_IRQ_CC2:
            case HAL_TIMER_IRQ_CC3:
            case HAL_TIMER_IRQ_CC4:
                params.nvic_irq_mask = (0x1u << 27u);
                break;
            case HAL_TIMER_IRQ_TRIGGER:
                params.nvic_irq_mask = (0x1u << 26u);
                break;
        ...
        params.nvic_reg_nr = 0u;
        params.nvic_irq_mask = (0x1u << 28u);
    } 
    else if (timer == TIM3) {
        params.nvic_reg_nr = 0u;
        params.nvic_irq_mask = (0x1u << 29u);
    } 
    else if (timer == TIM4) {
        params.nvic_reg_nr = 0u;
        params.nvic_irq_mask = (0x1u << 30u);
    } 
    else if (timer == TIM5) {
        params.nvic_reg_nr = 1u;
        params.nvic_irq_mask = (0x1u << 18u);
    } 
    else if (timer == TIM6) {
        params.nvic_reg_nr = 1u;
        params.nvic_irq_mask = (0x1u << 22u);
    } 
    else if (timer == TIM7) {
        params.nvic_reg_nr = 1u;
        params.nvic_irq_mask = (0x1u << 23u);
    } 
    else if (timer == TIM8) {
        params.nvic_reg_nr = 1u;
        assert(irq == HAL_TIMER_IRQ_UE || irq == HAL_TIMER_IRQ_CC1 || 
        ...
        switch(irq) {
            case HAL_TIMER_IRQ_UE:
                params.nvic_irq_mask = (0x1u << 12u);
                break;
            case HAL_TIMER_IRQ_CC1:
            case HAL_TIMER_IRQ_CC2:
            case HAL_TIMER_IRQ_CC3:
            case HAL_TIMER_IRQ_CC4:
                params.nvic_irq_mask = (0x1u << 14u);
                break;
            case HAL_TIMER_IRQ_TRIGGER:
                params.nvic_irq_mask = (0x1u << 13u);
                break;
            /* BRK not implemented yet */
        }
    } 
                                 ...
                                 hal_timer_polarity_t polarity)
{
    ...
    timer->CCER &= ~(((0x1u << bit_pos) << ch_pos));
    timer->CCER |= (((uint32_t) polarity << bit_pos) << ch_pos);
}
...
static void set_channel_ccreg(reg_tim_t *timer, 
                              ...
                              uint32_t reg)
{
    uint32_t clear_reg = ~(0xffu);
    assert(channel ==  HAL_TIMER_CH1 || channel == HAL_TIMER_CH2 || 
            channel == HAL_TIMER_CH3 || channel == HAL_TIMER_CH4 || 
            channel == HAL_TIMER_CH1N || channel == HAL_TIMER_CH2N || 
            channel == HAL_TIMER_CH3N);
    switch (channel) {
        case HAL_TIMER_CH2:
        case HAL_TIMER_CH2N:
            clear_reg <<= 8u;       /**< shift for ch. 2 */
            clear_reg |= 0xffu;     /**< don't overwrite ch. 1 */
            reg <<= 8u;             /**< shift for ch. 2 */
            __attribute__((fallthrough));
        case HAL_TIMER_CH1:         /**< default */
        case HAL_TIMER_CH1N:
        ...
        case HAL_TIMER_CH4:
            ...
            clear_reg |= 0xffu;     /**< don't overwrite ch. 3 */
