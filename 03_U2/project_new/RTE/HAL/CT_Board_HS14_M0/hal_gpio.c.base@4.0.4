...
void hal_gpio_reset(reg_gpio_t *port)
{
    if(port == GPIOA) {
        ...
        port->MODER = 0xa8000000u;
        port->OSPEEDR = 0x00000000u;
        port->PUPDR = 0x64000000u;
    }
    else if (port == GPIOB) {
        /* Reset GPIOB specific values */
        port->MODER = 0x00000280u;
        port->OSPEEDR = 0x000000c0u;
        port->PUPDR = 0x00000100u;
    } else {
        /* Reset other GPIO */
        port->MODER = 0x00000000u;
        port->OSPEEDR = 0x00000000u;
        port->PUPDR = 0x00000000u;
    }
    
    port->OTYPER = 0x00000000u;
    port->AFRL = 0x00000000u;
    port->AFRH = 0x00000000u;
    port->ODR = 0x00000000u;
}
...
void hal_gpio_init_input(reg_gpio_t *port, hal_gpio_input_t init)
{
    ...
    /* process mode */
    port->MODER &= ~create_pattern_mask(init.pins, 0x3u, 2u);
    port->MODER |= create_pattern_mask(init.pins, HAL_GPIO_MODE_IN, 2u);

    /* process pull up/down resitors */
    port->PUPDR &= ~create_pattern_mask(init.pins, 0x3u, 2u);
    port->PUPDR |= create_pattern_mask(init.pins, (uint8_t) init.pupd, 2u);
}
...
void hal_gpio_init_output(reg_gpio_t *port, hal_gpio_output_t init)
{
    ...
    /* process port speed */
    port->OSPEEDR &= ~create_pattern_mask(init.pins, 0x3u, 2u);
    port->OSPEEDR |= create_pattern_mask(init.pins, (uint8_t) init.out_speed, 2u); 
                             ...
                             hal_gpio_output_t init)
{
    ...
    /* process af type */
    port->AFRL &= ~create_pattern_mask(init.pins, 0xfu, 4u);
    port->AFRL |= create_pattern_mask(init.pins, (uint8_t) af_mode, 4u);
    port->AFRH &= ~create_pattern_mask((init.pins >> 8), 0xfu, 4u);
    port->AFRH |= create_pattern_mask((init.pins >> 8), (uint8_t) af_mode, 4u);
}
    ...
    for (exti_line = 0u; exti_line < 16u; exti_line++) {
        if (pins & (0x1u << exti_line)) {
            ...
            if (status == ENABLE) {                
                ...
                EXTI->IMR &= ~(0x1u << exti_line);
                if (nvic_bank == 0u) {
                    NVIC->ICER0 |= ((exti_line <  5u) ? (0x1u << (exti_line + NVIC_OFFSET_1_4)) :
                                              (exti_line < 10u) ? NVIC_OFFSET_5_9 : NVIC_OFFSET_10_15);                    
                } else if (nvic_bank == 1u) {
                    NVIC->ICER1 |= ((exti_line <  5u) ? (0x1u << (exti_line + NVIC_OFFSET_1_4)) :
                                              (exti_line < 10u) ? NVIC_OFFSET_5_9 : NVIC_OFFSET_10_15);                  
                } else if (nvic_bank == 2u) {
                    NVIC->ICER2 |= ((exti_line <  5u) ? (0x1u << (exti_line + NVIC_OFFSET_1_4)) :
                                              (exti_line < 10u) ? NVIC_OFFSET_5_9 : NVIC_OFFSET_10_15);
                }
            }
        }
                                    ...
                                    uint8_t pattern_bit_width)
{
    ...
    if (pattern_bit_width == 2u || pattern_bit_width == 4u) {
        ...
        for (pos = 0u; pos < end; pos++) {
            if (pins & pin1_mask) {
                mask |= ((uint32_t) pattern << (pos * pattern_bit_width));
            }
            pins >>= 1u;
        }
    } else {
