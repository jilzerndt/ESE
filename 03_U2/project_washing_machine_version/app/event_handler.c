...
event_t eh_get_event(void)
{
    ...
    if (edge_negative & MASK_BUTTON_STOP)
    {
        event = BUTTON_STOP;
    } 
    else if (edge_negative & MASK_BUTTON_WASH)
    {
        event = BUTTON_WASH;
    } 
    else if (edge_negative & MASK_BUTTON_SPIN)
    {
        event = BUTTON_SPIN;
    } 
    else if (edge_positive & MASK_DOOR_CLOSED)
    {
        event = DOOR_OPENED;
    }
    else if (edge_negative & MASK_DOOR_CLOSED)
    {
        event = DOOR_CLOSED;
    }
    else if (edge_negative & MASK_FLOATER_HIGH)
    {
        event = FLOATER_HIGH;
    }
    else if (edge_negative & MASK_FLOATER_LOW)
    {
        event = FLOATER_LOW;
    }
